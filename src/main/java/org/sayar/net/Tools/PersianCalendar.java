...
public class PersianCalendar implements Serializable  {
	...
	public static String getPersianDate(Date d) {
		...
		return getPersianDate(year, (gc.get(Calendar.MONTH)) + 1,
				gc.get(Calendar.DAY_OF_MONTH))+"-"+d.getHours()+":"+d.getMinutes();
	}
	...
	public static Date getGregorainDate(String persianDate){
		int[] dataParts = persianDateSplit(persianDate);
		return getGregorainCalendar(dataParts[0],dataParts[1],dataParts[2],dataParts[3],dataParts[4],0).getTime();
	}
	...
	public static int[] persianDateSplit(String persianDate){

		if (persianDate.length() == 10){
			persianDate+= "-0:0";
		}

		int slash = persianDate.indexOf("/");
		int slash2 = persianDate.lastIndexOf("/");

		int dash = persianDate.indexOf("-");
		int dot = persianDate.indexOf(":");
		int end = persianDate.length();

		int[] result = new int[5];

		result[0] = Integer.parseInt(persianDate.substring(0,slash));
		result[1] = Integer.parseInt(persianDate.substring(slash+1,slash2));
		result[2] = Integer.parseInt(persianDate.substring(slash2+1,dash));
		result[3] = Integer.parseInt(persianDate.substring(dash+1,dot));
		result[4] = Integer.parseInt(persianDate.substring(dot+1,end));
		return result;

	}

	public Date getDate(String persianDate){
		int[] slice = persianDateSplit(persianDate);

		Date date = new Date();
		date.setYear(slice[0]);
		date.setMonth(slice[1]);
		date.setDate(slice[2]);
		date.setHours(slice[3]);
		date.setMinutes(slice[4]);

		return date;
	}

	public long getTowTowDateDayDiffrence(String date1,String date2){
		long date1Time = new PersianCalendar().getDate(date1).getTime();
		long date2Time = new PersianCalendar().getDate(date2).getTime();
		long difference = date1Time - date2Time;
